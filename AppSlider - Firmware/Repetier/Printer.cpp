...
void Printer::updateDerivedParameter()
{
    ...
    minimumSpeed = ( 0.1 / 3.0 ) * sqrt(2.0f / (axisStepsPerMM[X_AXIS] * 0.1)); //accel * sqrt(2.0f / (axisStepsPerMM[X_AXIS]*accel));
...
uint8_t Printer::setDestinationSteps(GCode *com)  // absolute Koordinaten
{
    register int32_t p;
    float x, y, z;
    ...
    return !com->hasNoXYZ(); // ignore unproductive moves
}

uint8_t Printer::setDestinationSteps(GCode *com, float dx) // absolute Koordinaten mit DeltaX-Bewegung
{
    register int32_t p;
    float x, y, z;
    if(com->hasX() || (com->hasE() && com->hasP())) currentPosition[X_AXIS] = (lastCmdPos[X_AXIS] += convertToMM(dx));
    if(com->hasY()) currentPosition[Y_AXIS] = (lastCmdPos[Y_AXIS] += convertToMM(com->Y));
    if(com->hasZ()) currentPosition[Z_AXIS] = (lastCmdPos[Z_AXIS] += convertToMM(com->Z));
    ...
    return !com->hasNoXYZ() || (com->hasE() && com->hasP());/*|| (com->hasE() && destinationSteps[E_AXIS] != currentPositionSteps[E_AXIS])*/ // ignore unproductive moves
}
#endif
