...
uint32_t Commands::getDelayTime(GCode *com)
{
  uint32_t codenum = 0; //throw away variable
...
void Commands::pause(uint32_t delayTime)
{
  Commands::waitUntilEndOfAllMoves();
  delayTime += HAL::timeInMilliseconds();  // keep track of when we started waiting
  while((uint32_t)(delayTime-HAL::timeInMilliseconds())  < 2000000000 )
  {
    GCode::readFromSerial();
...
void Commands::camShot(float startVerzoegerung, long fokusTime, long shootTime, float nachgeschalteteVerzoegerung, bool isFokusShootSame)
{
  if(startVerzoegerung > 0) // Startverzögerung
  {
    Commands::pause(startVerzoegerung);
  }
  if(fokusTime > 0) // Fokusieren
  {
    pinMode(FOCUS_PIN, OUTPUT);
    digitalWrite(FOCUS_PIN, HIGH);            
    Commands::pause(fokusTime);
    digitalWrite(FOCUS_PIN, LOW);
  }
  if(shootTime > 0) // Schnappschuss
  {
    if(isFokusShootSame)
    {
      pinMode(FOCUS_PIN, OUTPUT);
    ...
    Commands::pause(shootTime);

    if(isFokusShootSame)
      digitalWrite(FOCUS_PIN, LOW);
    digitalWrite(SHOOT_PIN, LOW);
  }
  if(nachgeschalteteVerzoegerung > 0) // Nachgeschaltete Pause
  {
    Commands::pause(nachgeschalteteVerzoegerung);  
  }
}
#endif
...
#ifdef SHOOT_SUPPORT
    case 750: //M750 - Fokus, Shoot, Zeitraffer oder Shoot and Move
      { // diese Klamme ist aufgrund der Variablendefinition in einem Case notwendig

        //Com::printFLN(PSTR("Nachricht "), value);  // Nachricht senden

        // Erhalte Fokus- und Shoot-Zeit
        long r = 0, s = 0;
        if(com->hasR()) r = com->R;
        if(com->hasS()) s = com->S;
        if(r == 0 && s == 0)
        ...
        // Falls bei Shoot der Fokus nicht gleichzeitig betätigt werden soll
        bool isFokusShootSame = true;
        if(com->hasT()) isFokusShootSame = (bool)com->T;  // StandBilder

        // Erhalte vor- und nachgeschaltete Verzögerung
        float i = 0, j = 0;
        if(com->hasI())
          i = com->I;
        if(com->hasJ())
        ...
        float x; // EndPosition in X-Richtung
        float e; // deltaBewegung
        long cp = 0;  // insgesamt geschossene Bilder
        if(com->hasX()) // Shoot and Move
        {
          x = com->X;
          float deltaStrecke = x - Printer::currentPosition[X_AXIS];
          if(com->hasE())
          {  
            e = com->E;
            if(deltaStrecke < 0)  // das versendete Vorzeichen von E ist egal und wird passend gesetzt
              e = -abs(e);
            cp = abs(deltaStrecke / e);
          }
          else if(com->hasP())
          {
            cp = com->P - 1;
        ...
        else
        {
          if(com->hasP())
          {
            ...
            {
              e = com->E;
              x = Printer::currentPosition[X_AXIS] + (((float)cp)*e);
              if(x < 0)
              {
                x = 0;
                e = -Printer::currentPosition[X_AXIS] / cp;
              }
              else if(x > X_MAX_LENGTH)
              {
               x = (float)X_MAX_LENGTH;
               e = ((float)X_MAX_LENGTH - Printer::currentPosition[X_AXIS]) / cp;
              }
            }
            else 
            {
        ...
        if(cp >= 0)
        {
          Com::printFLN(PSTR("Shot "), nr);
            
          Commands::waitUntilEndOfAllMoves();
          Commands::camShot(i, r, s, j, isFokusShootSame);  // Cam-Steuerung   
             
          nr--;
        }
        for (long i_ = 0; i_ < cp; ++i_)  // eins weniger damit man am Ende noch ein Foto schießt
        {
          Com::printFLN(PSTR("Shot "), nr - i_);   
          
          // Bewegen
          if(e != 0 && Printer::setDestinationSteps(com, e)) 
            PrintLine::queueCartesianMove(ALWAYS_CHECK_ENDSTOPS, true);
          
          Commands::waitUntilEndOfAllMoves(); 
          Commands::camShot(i, r, s, j, isFokusShootSame);  // Cam-Steuerung
        }
        // Bewege zur genauen Endposition
        if(e != 0 && Printer::setDestinationSteps(com)) // For X Y Z F
          PrintLine::queueCartesianMove(ALWAYS_CHECK_ENDSTOPS, true);
       ...
       /* }
        ...
        {
          if(com->hasP()) // Insgesamt geschossene Bilder
          {
            long p = com->P;
            for(long i = 0; i < p; ++i)
            {
              Commands::camShot(i, r, s, j, isFokusShootSame);
            }
          }
        }*/

          Com::printFLN(PSTR("Shot "), 0);
        }
        break;
#endif 
